...
@section('title', 'Login Pasien - MediHub')
@section('auth_page_class', 'mediq-signin-page')

@section('content')
    <div class="mediq-auth-body">
        <h1 class="mediq-auth-heading">Selamat Datang, Pasien!</h1>
        <p class="mediq-auth-subtitle">Masuk ke akun Anda untuk membuat janji temu kesehatan.</p>

        <p class="mediq-auth-switch">Belum memiliki akun? <a href="{{ route('register-pasien') }}">Daftar sekarang</a></p>
        ...
        <form id="firebase-sign-in-form" class="mediq-auth-form" method="POST" action="{{ route('firebase.session.login') }}"
            ...
            <div class="mediq-field">
                ...
                <input id="email" name="email" type="email" value="{{ old('email') }}" class="mediq-input"
                    placeholder="pasien@medihub.com" required autofocus>
            </div>
...
@section('page-scripts')
    @vite('resources/js/auth-pasien/pasien-sign-in.js')
@endsection
